...
/*   Updated: 2019/07/22 12:59:34 by arsciand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	exec_handler_bin_error(t_core *shell)
{
	...
	** tokens[0] cames from an expansion variable
	...
	{
		ft_mprintf(STDERR_FILENO, "42sh: %s: is a directory\n",
			shell->tokens[0]);
	}
...
static void	exec_handler_perm_error(t_core *shell)
{
	...
	if (access(shell->tokens[0], F_OK) == -1)
		dmprintf(STDERR_FILENO,
			"minishell: %s: No such file or directory\n", shell->tokens[0]); /* /!\ */
	else
		dmprintf(STDERR_FILENO, "minishell: %s: Permission denied\n",
			shell->tokens[0]);	/* /!\ */
}

void		exec_handler(t_core *shell, uint8_t handler)
{
